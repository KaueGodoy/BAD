...
public class EnemySkeleton extends Sprite {
	...
	// animation
	private Animation<TextureRegion> skeletonIdle, skeletonRight, skeletonLeft, skeletonAttack;
	...
	public EnemySkeleton(Animation<TextureRegion> skeletonIdle, Animation<TextureRegion> skeletonRight, Animation<TextureRegion> skeletonLeft, Animation<TextureRegion> skeletonAttack, TiledMapTileLayer collisionLayer) {
		super(skeletonIdle.getKeyFrame(0));
		this.skeletonIdle = skeletonIdle;
		this.skeletonRight = skeletonRight;
		this.skeletonLeft = skeletonLeft;
	...
	public void movementUpdate(float delta) {
		...
		if(MainGameScreen.isEnemyAlive()) {
			if(collidesBottomDeath()) {
				if(MainGameScreen.iFrameTimer > MainGameScreen.MAX_IFRAME_TIME) {
					Player.health -= 0.1f;
					MainGameScreen.iFrameTimer = 0;
				}
				health = 0;
			}
		}
...
//----------------------------------- MOVEMENT -------------------------------------
	...
	public void checkMovement() {
		...
		if(isMoving()) {
			if(isMovingOnCooldown())
				movingTimer = 0;
			
			if(canMoveRight()) {
				isRight();
				if(canMoveRightIsOnCooldown())
					travelSpaceTimer = 0;
			}
			else
				isLeft();	
		...
		if(MainGameScreen.attackingRadius) {
			isAttacking();
		}
	...
	public void isIdle() {
		setRegion(skeletonIdle.getKeyFrame(animationTime));
		velocity.x = 0;
	}
	
	public void isRight() {
		setRegion(skeletonRight.getKeyFrame(animationTime));
		velocity.x = speed;
	}
	
	public void isLeft() {
		setRegion(skeletonLeft.getKeyFrame(animationTime));
		velocity.x = -speed;
	}
	
	public void isAttacking() {
		setRegion(skeletonAttack.getKeyFrame(animationTime));
	}
